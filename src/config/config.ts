export default {
  debug: false,
  port: 3000,
  host: "localhost",
  dbUri: "mongodb://localhost:27017/cashback-api",
  apiExternalCashback: "https://mdaqk8ek5j.execute-api.us-east-1.amazonaws.com/v1/cashback",
  rootResellerCpf: "15350946056",
  jwtSecret: `-----BEGIN RSA PRIVATE KEY-----
  
  -----END CERTIFICATE-----`
};